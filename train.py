...
class EMAWeights:
    def __init__(self, named_params, decay=0.99):
        self.decay = float(decay)
        self.num_updates = 0
        self.shadow = {}
        self.backup = {}
        self._register(named_params)

    def _key(self, name, p):
        return getattr(p, "original_name", None) or name

    @torch.no_grad()
    def _register(self, named_params):
        # register trainable float params (lazy-safe)
        ...
        if is_main_process():
            print(f"[EMA] tracking {len(self.shadow)} tensors", flush=True)

    @torch.no_grad()
    def update(self, named_params):
        # if original_name appears late, this rescues empty EMA
        ...
        for name, p in named_params:
            ...
            t.mul_(d).add_(p.detach(), alpha=one_minus)

        self.num_updates += 1

    @torch.no_grad()
    def swap_in(self, named_params):
        self.backup = {}
        for name, p in named_params:
            if (not p.requires_grad) or (not torch.is_floating_point(p)):
                continue
    ...
    @torch.no_grad()
    def swap_out(self, named_params):
        for name, p in named_params:
            if (not p.requires_grad) or (not torch.is_floating_point(p)):
                continue
            k = self._key(name, p)
    ...
    def state_dict(self):
        # store on CPU to make checkpoint portable
        return {
            "decay": self.decay,
    ...
    def load_state_dict(self, sd):
        ...
        self.shadow = {k: v for k, v in shadow.items()}
        self.backup = {}
...
def evaluate_with_ema(
    model, model_engine, eval_dataloaders, tb_writer,
    step, eval_gradient_accumulation_steps,
    disable_block_swap, ema, pipeline_model
):
    if ema is None:
        evaluate(
            model, model_engine, eval_dataloaders,
            tb_writer, step,
    ...
    if is_main_process():
        print("[EMA] swap in for eval", flush=True)

    ema.swap_in(pipeline_model.named_parameters())
    try:
        evaluate(
            model, model_engine, eval_dataloaders,
            tb_writer, step,
        ...
        ema.swap_out(pipeline_model.named_parameters())
...
if __name__ == '__main__':
    ...
    training_type = config.get('model', {}).get('train_type', 'normal_training')
    ...
    elif model_type == 'sd3' and training_type == "sd3_light_pretrain":
        from models import sd3_light

        model_cfg = config["model"]

        model = sd3_light.LightSD3Pipeline.load_from_pretrained(
            model_cfg["diffusers_path"],
            dtype=model_cfg["dtype"],
            transformer_dtype=model_cfg.get("transformer_dtype", None),
            extra_model_config=model_cfg,
        )
    elif model_type == 'sd3':
    ...
    if sd3_streaming:
        ...
        manifest_builder = dataset_util.SD3LightManifestBuilder(dataset_config, model, model.name, cache_root, shard_size=config.get('sd3_shard_size', 512))
    ...
    # -------------------------
    # EMA (only for sd3_light_pretrain)
    # -------------------------
    ema = None
    ema_cfg = config.get("ema", {})
    ema_enabled = (training_type == "sd3_light_pretrain") and bool(ema_cfg.get("enable", True))
    EMA_DECAY = float(ema_cfg.get("decay", 0.99))
    EMA_UPDATE_EVERY = int(ema_cfg.get("update_every", 100))


    denoiser = model.diffusers_pipeline.transformer
    params = list(denoiser.named_parameters())

    # TEST
    print(f"[EMA-DEBUG] total named_parameters from denoiser = {len(params)}")
    ...
    print(
        "[EMA-DEBUG] "
        f"requires_grad={cnt_requires_grad}, "
        f"has_data={cnt_has_data}, "
        f"non_meta={cnt_non_meta}"
    )
    ...
    print(f"[EMA-DEBUG] denoiser params meta={cnt_meta} / total={cnt_total}")
    ...
    while True:
        ...
        if ema is not None and step % EMA_UPDATE_EVERY == 0:
            ...
            if is_main_process():
                print(
                    f"[EMA] step={step}, updates={ema.num_updates}, "
                    f"tracked={len(ema.shadow)}",
                    flush=True
                )
